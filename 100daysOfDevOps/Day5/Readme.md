...
### Damage Containment:
If a web server is compromised, SELinux prevents the attacker from accessing user data or other system parts, even if they gain root privileges.

### Mandatory Access Control(MAC): 
Unlike traditional discretionary Access Control(DAC), where users can change file permissions, SELinux policies are defined by admins and cannot be overridden

### Preventing Privilege Escalation: 
SELinix secures against malware that attempts to gain higher privileges(eg. from user to root)

### Compliance: 
For organizations like finance, healthcare, MAC is required.

### Container Security: 
It isolates containerized applications to prevent escapes that could compromise the host system.
